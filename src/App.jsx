      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 print-full-width">
        ...
        {isPrintMode && (
          <div className="fixed inset-0 bg-white z-[999] overflow-y-auto p-8 flex flex-col no-print animate-in fade-in duration-200">
            <div className="flex flex-col sm:flex-row justify-between items-center bg-slate-900 text-white p-4 rounded-xl mb-8 gap-4 shadow-lg"><div className="flex items-center gap-3"><AlertCircle className="text-yellow-400" size={24} /><div className="text-left"><p className="font-semibold text-sm">Modus Pratinjau Cetak Aktif</p><p className="text-xs text-slate-400">Gunakan pintasan keyboard untuk mencetak kertas jika dialog browser terblokir.</p></div></div><div className="flex gap-2 w-full sm:w-auto"><button onClick={() => { try { window.print(); } catch(e) {} }} className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1 cursor-pointer"><Printer size={14} /> Cetak (Ctrl+P / Cmd+P)</button><button onClick={() => setIsPrintMode(false)} className="flex-1 sm:flex-none bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer">Kembali ke Aplikasi</button></div></div>
